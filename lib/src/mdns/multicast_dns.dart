...
import 'lookup_resolver.dart';
import 'native_protocol_client.dart';
import 'packet.dart';
...
class MDnsClient {
  ...
  Future<void> start({
    ...
    final RawDatagramSocket incoming = await _rawDatagramSocketFactory(
      ...
      selectedMDnsPort,
      reuseAddress: CastConstants.reuseAddress,
      reusePort: CastConstants.reusePort,
      ttl: CastConstants.hostTTL,
    );
    ...
    incoming.listen((RawSocketEvent event) => _handleIncoming(event, incoming),
      ...
      print("mDNS: Incoming Error: $error");
    });
    _started = true;
    _starting = false;
  }
  ...
  Stream<T> lookup<T extends ResourceRecord>(
    ...
    if (cached.isNotEmpty) {
      ...
      print("mDNS: Returning cached results for ${query.fullyQualifiedName}");
      print("cached: $cached");
  ...
  void _handleIncoming(RawSocketEvent event, RawDatagramSocket incoming) {
    print("mDNS: Incoming event: $event");
    if (event == RawSocketEvent.read) {
      ...
      if (response != null) {
        print(
            "Published Responses: ${response.fold("", (previousValue, element) => "$previousValue ${element.toString()}")}");
